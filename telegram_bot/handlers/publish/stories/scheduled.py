"""
Обработчики для планирования публикации Stories
"""
...
def start_schedule_story(update, context):
    """Начинает процесс планирования истории используя существующий механизм публикации"""
    # Устанавливаем флаг что это запланированная история
    context.user_data['is_scheduled_post'] = True
    context.user_data['publish_type'] = 'story'
    
    # Используем существующий механизм выбора аккаунтов для историй
    from .handlers import start_story_publish
    return start_story_publish(update, context)


def handle_schedule_time_input(update, context):
    """Обработка ввода времени для планирования истории"""
    ...
    try:
        ...
        task_ids = story_handler.create_publish_tasks(context, scheduled_time)
        
        if task_ids:
            message = f"✅ Запланировано историй: {len(task_ids)}\n\n"
            message += f"📅 Время публикации: {format_scheduled_time(scheduled_time)}\n\n"
            message += "Истории будут опубликованы автоматически в указанное время."
        ...
        # Очищаем данные
        story_handler.cleanup_user_data(context)
...
def schedule_story_publish(update, context, scheduled_time, user_id):
    """Создает запланированные задачи для публикации историй"""
    from .handlers import story_handler
    ...
    try:
        ...
        logger.error(f"Error scheduling story publish: {e}")
